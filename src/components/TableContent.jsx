...
const TableContent = ({
  ...
  // Define condition for showing L3 column
  const showL3Column = activeRole === "l3" || activeRole === "Supervisor";
  ...
  const sortedLeads = useMemo(() => {
    ...
    return [...leads].sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case "lastCall":
          const aLastRemark =
            a?.student_remarks?.[a.student_remarks.length - 1];
          const bLastRemark =
            b?.student_remarks?.[b.student_remarks.length - 1];
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        align: "center",
        render: (value) => (
          <Tag className="px-2 py-1 text-xs">{value || 0}</Tag>
        ),
      },
      {
        title: "Lead ID",
        ...
        render: (value, record) => {
          ...
          return (
            <div className="space-y-1 flex">
              ...
              {isOverdue && (
                <div className="text-xs text-red-600 font-medium pl-5">
                  Overdue
                </div>
              )}
              {isToday && (
      ...
      },
      {
        ...
        // L2 column always visible or you can add condition if needed
      },
      {
        title: "Counsellor (L3)",
        ...
        // Show L3 column only for l3 role and Supervisor
        hidden: !showL3Column,
      },
      {
        title: "Campaign",
...
console.log(activeRole, "active role in table content");
  const dataSource = useMemo(() => {
    ...
    return sortedLeads.map((lead) => {
      const lastRemark =
        lead?.student_remarks?.[lead.student_remarks.length - 1];
      return {
        key: lead.student_id,
        ...lead,
        createdAt:
        ...
        remarks: Number(
          activeRole === "l2" ||
            ...
            : lead.total_remarks_l3 || 0,
        ),
        source: lead?.lead_activities?.[0]?.source || "Unknown",
        campaign: lead?.lead_activities?.[0]?.utm_campaign || "",
        counsellorL2:
          lead.assignedCounsellor?.counsellor_name ||
          lead.counsellorDetails?.name ||
          "",
        counsellorL3:
          lead.assignedCounsellorL3?.counsellor_name ||
          lead.counsellorL3Details?.name ||
          "",
